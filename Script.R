...
nsim = 1  ## set the number of simulation we want for betas

## Create dataframes to save the beta values
num_columns = ncol(X_all)
...
colnames(nsim_betas_ridge)   = colnames(X_all)
colnames(nsim_betas_lasso)   = colnames(X_all)
colnames(nsim_betas_elast)   = colnames(X_all)
colnames(nsim_betas_forest) = colnames(X_all)
...
calculate_AUC    = function(y.test, prob.test){
  dt              =        0.01
...
time_start= rep(NA,nsim)
time_end  = rep(NA,nsim)
...
for (i in 1:nsim){
  ...
  #K-fold cross validation for Elastic Net
  time.start    = Sys.time()   ## time ridge for tuning
  ...
  auc_forest[i,5]    =   Sys.time() - time.start
  auc_forest[i,6]    =   auc_forest[i,5] 
  ...
  time_end[i] = Sys.time() ## end time for each simulation
  print(paste('Time Elapsed: ',time_end[i] - time_start[i] ))
}

write.csv(nsim_betas_ridge ,"betas_ridge.csv")
write.csv(nsim_betas_lasso ,"betas_ridge.csv")
write.csv(nsim_betas_elast ,"betas_ridge.csv")
write.csv(nsim_betas_forest ,"betas_ridge.csv")

# I then combined them on excel using vlookup for each feature so that
